...
fn make_create_modal() -> Modal {
  let state = tag_modal.hidden()
  let #(state, _, _) = tag_modal.update(state, CreateRequested, [])
  state
}

fn make_edit_modal(tag: Tag) -> Modal {
  let state = tag_modal.hidden()
  let #(state, _, _) = tag_modal.update(state, EditRequested(tag), [])
  state
}

fn modal_with_tag_name(state: Modal, tag_name: String) -> Modal {
  let #(state, _, _) = tag_modal.update(state, NameChanged(tag_name), [])

  state
}

fn try_validate(state: Modal, tags: List(Tag)) -> Modal {
  let #(state, _, _) = tag_modal.update(state, SaveRequested, tags)
  state
}
...
pub fn validate_trims_the_request_name_but_not_the_field_test() {
  ...
  color |> should.equal(tag_modal.default_color)
}
...
pub fn set_name_records_too_long_error_test() {
  let modal =
    make_create_modal()
    |> modal_with_tag_name(string.repeat("a", tag_modal.max_name_length + 1))
...
pub fn typing_keeps_the_space_the_user_just_typed_test() {
  ...
  let #(state, _, _) =
    tag_modal.update(make_create_modal(), NameChanged("Food & "), [])

  let assert Active(form: Form(name: field.Valid(value: _, input:), ..), ..) =
    state
  input |> should.equal("Food & ")

  let #(typed, _, _) = tag_modal.update(state, NameChanged("Food & Drink"), [])
  let assert Active(form: Form(name: field.Valid(value: _, input:), ..), ..) =
    typed
...
pub fn create_tag_workflow_test() {
  ...
  let assert #(named, _, NoChange) =
    tag_modal.update(modal, NameChanged("Coffee"), [])
...
pub fn save_failure_then_fix_then_retry_test() {
  ...
  let assert #(still_errored, _, NoChange) =
    tag_modal.update(errored, NameChanged("Tea"), [])
