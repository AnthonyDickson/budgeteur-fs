...
fn update_inner(
  ...
  case msg {
    ...
    UserRequestedRuleDelete(rule) -> #(
      Model(..model, rule_delete_modal: rule_delete_modal.open(rule)),
    ...
    UserConfirmedRuleDelete -> {
      case model.rule_delete_modal {
        rule_delete_modal.Confirming(rule) -> {
...
fn selected_tag_name(model: Model) -> String {
  case model.selected_tag {
    None -> ""
    Some(id) ->
      case list.find(model.tags, fn(tag) { tag.id == id }) {
          ...
          html.div(
            ...
            [
              html.div(
                ...
                [
                  html.div([attribute.class("flex items-center gap-2")], [
                    ...
                    html.h2(
                      [attribute.class("text-sm font-semibold text-gray-900")],
              ...
              ..case list.is_empty(rules) {
                ...
                False -> [column_headers(), rules_list(rules)]
              }
            ],
          )
        }
...
fn rules_list(rules: List(Rule)) -> Element(Msg) {
  html.ul(
    [attribute.class("flex-1 divide-y divide-gray-100 overflow-y-auto")],
          ...
          html.div([attribute.class("ml-auto flex items-center gap-2")], [
            ...
            html.button(
              [
                ...
                event.on_click(UserRequestedRuleDelete(rule)),
              ],
              [html.text("Delete")],
            ),
          ]),
