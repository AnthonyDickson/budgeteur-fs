...
defined in `Data/Migrations/`. The header comment of the first migration documents the column type conventions — the
`GUID`, `BOOLEAN`, `DATETIME` etc. hints are not real SQLite types but drive SqlHydra's F# codegen. See
